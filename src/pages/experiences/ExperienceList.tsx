...
import { MapPin, Accessibility, Dog, DollarSign, Clock, Phone, Mail, Globe, ExternalLink, Share2, X } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-primary-50 pt-20">
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        {/* Detail panel */}
        {selectedSlug && (
          <ExperienceDetailPanel slug={selectedSlug} onClose={() => setSelectedSlug(null)} />
        )}
          ...
          {loadingExps
            ...
            : experiences?.map((exp) => (
                <ExperienceCardItem
                  ...
                  isSelected={selectedSlug === exp.slug}
                  onHover={() => setHoveredId(exp.id)}
                  onLeave={() => setHoveredId(null)}
                  onClick={() => setSelectedSlug(selectedSlug === exp.slug ? null : exp.slug)}
                />
              ))}
        </div>
...
function ExperienceDetailPanel({ slug, onClose }: { slug: string; onClose: () => void }) {
  const { data: exp, isLoading } = useExperienceBySlug(slug);

  if (isLoading) {
    return (
      <div className="mt-6 rounded-xl bg-accent-50 p-6 shadow-lg">
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-xl bg-accent-50 shadow-lg">
      {/* Header with close */}
      <div className="flex items-start justify-between border-b border-primary-100 p-6">
        <div>
          {exp.categoria && (
            <Badge className="mb-2 text-accent-50" style={{ backgroundColor: exp.categoria.cor ?? "#7b9669" }}>
              {exp.categoria.nome}
            </Badge>
          )}
          <h2 className="text-2xl font-bold text-primary-800">{exp.nome}</h2>
        </div>
        <button onClick={onClose} className="rounded-full p-2 text-accent-400 transition-colors hover:bg-primary-100 hover:text-primary-700">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="p-6">
        {/* Image */}
        {exp.imagem_destaque && (
          <img src={exp.imagem_destaque} alt={exp.nome} className="mb-6 h-64 w-full rounded-xl object-cover" />
        )}
        ...
        {/* Description */}
        {exp.descricao && (
          <p className="whitespace-pre-line leading-relaxed text-primary-700">{exp.descricao}</p>
        )}

        <Separator className="my-6 bg-primary-100" />
